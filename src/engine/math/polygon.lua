...
function Polygon:scale(sx, sy, ox, oy)
	for i = 1, #self.vertices, 2 do
		self.vertices[i], self.vertices[i + 1] =
			math.scale_point(self.vertices[i], self.vertices[i + 1], sx or 1, sy or sx or 1, ox or self.cx or 0, oy or self.cy or 0)
	end
end
...
function Polygon:get_size()
	...
	for i = 1, #self.vertices, 2 do
		if self.vertices[i] < min_x then
			min_x = self.vertices[i]
		end
		if self.vertices[i] > max_x then
			max_x = self.vertices[i]
		end
		if self.vertices[i + 1] < min_y then
			min_y = self.vertices[i + 1]
		end
		if self.vertices[i + 1] > max_y then
...
function Polygon:is_colliding_with_circle(circle)
	return mlib.polygon.isCircleCompletelyInside(circle.x, circle.y, circle.rs, self.vertices)
		or mlib.circle.isPolygonCompletelyInside(circle.x, circle.y, circle.rs, self.vertices)
		or mlib.polygon.getCircleIntersection(circle.x, circle.y, circle.rs, self.vertices)
end
...
function Polygon.merge_polygons(polygons)
	...
	for _, polygon in ipairs(polygons) do
		paths:add(polygon_to_path(polygon.vertices))
	end
	cl:addPaths(paths, "subject")
	local out = cl:execute("union")
	local out_polygons = {}
	for i = 1, out:size() do
		table.insert(out_polygons, path_to_polygon(out:get(i)))
	end
	return out_polygons
end
