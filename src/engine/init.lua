...
function engine_run(config)
	if not web then
		...
		if state.sx and state.sy then
			...
			windowArgs.width = state.sx * gw
			windowArgs.height = state.sy * gh
		else
			state.sx, state.sy = sx, sy
			windowArgs.width = window_width
			windowArgs.height = window_height
		end
	...
	local last_window_width, last_window_height = love.graphics.getDimensions()
...
function setWindow(args)
	for _, key in ipairs({ "width", "height", "vsync", "msaa", "display" }) do
		if args[key] ~= nil then
			state[key] = args[key]
		end
	end

	love.window.setMode(state.width or 960, state.height or 540, {
		vsync = state.vsync,
		msaa = state.msaa or 0,
