...
function Timing_Line:draw()
	...
	local gray_line_x = self.x - self.w / 2 + ((#self.timings == self.max_beats and #self.timings + 1 or #self.timings) * self.w / (self.max_beats + 1))
	...
	for i = 1, self.max_beats, 1 do
		...
		tick_color = i <= #self.timings and (self.timings[i][1].color or line_color) or _G["black"][0]
