...
function Group:update(dt)
	...
	self.cells = {}
	-- for _, object in ipairs(self.objects) do -- removing goto cuz emscripten/lovejs use lua 5.1 which doesn't have goto
	...
	-- end
	for _, object in ipairs(self.objects) do
		local cx = math.floor(object.x / self.cell_size)
		local cy = math.floor(object.y / self.cell_size)

		if cx == cx and cy == cy then
	...
	for i = #self.objects, 1, -1 do
		if self.objects[i].dead then
			...
			table.delete(self.objects.by_class[getmetatable(self.objects[i])], function(v)
				return v.id == self.objects[i].id
			end)
...
function Group:draw(scroll_factor_x, scroll_factor_y)
	if self.camera then
		self.camera:attach(scroll_factor_x, scroll_factor_y)
	end
	for _, object in ipairs(self.objects) do
...
function Group:draw_all_except(classes, scroll_factor_x, scroll_factor_y)
	...
	for _, object in ipairs(self.objects) do
		if not table.any(classes, function(v)
			return object:is(v)
		end) and not object.hidden then
			object:draw()
		end
	end
	if self.camera then
...
function Group:get_objects_by_class(class)
	if not self.objects.by_class[class] then
		...
		return table.shallow_copy(self.objects.by_class[class])
	end
end
					...
					for _, object in ipairs(cell_objects) do
						if object_types then
							if not table.any(exclude_list, function(v)
								...
								if
									table.any(object_types, function(v)
										return object:is(v)
									end)
									and object.shape
									and object.shape:is_colliding_with_shape(shape)
								then
...
function Group:get_closest_object(object, select_function)
	if not select_function then
		select_function = function(o)
			return true
		end
	end
