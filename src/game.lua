...
function Game:draw()
	...
	if self.start_time and self.start_time > 0 and not self.choosing_passives then
		...
		graphics.print_centered(tostring(self.start_time), fat_font, gw / 2, gh / 2 - 48, 0, 1, 1, nil, nil,
	...
	if state.run_timer then
		graphics.print_centered(math.round(run_time, 0), fat_font, self.x2 - 12, self.y2 + 16, 0, 0.6, 0.6, nil, nil,
			fg[0])
	end
	camera:detach()
...
function CharacterHP:draw()
	...
	graphics.rectangle(self.x, self.y - 2, 14, 4, 2, 2,
		self.parent.dead and bg[5] or (self.hfx.hit.f and fg[0] or self.parent.color[-2]), 2)
	...
	if not self.parent.dead then
		graphics.line(self.x - 8, self.y + 5, self.x - 8 + 15.5 * self.cooldown_ratio, self.y + 5,
			self.hfx.hit.f and fg[0] or self.parent.color[-2], 2)
	end
	graphics.pop()
	...
	if state.cooldown_snake then
		...
		if not p.dead then
			...
			graphics.line(p.x - 4, p.y + 8, p.x - 4 + 8 * self.cooldown_ratio, p.y + 8,
				self.hfx.hit.f and fg[0] or self.parent.color[-2], 2)
		end
		graphics.pop()
	end
