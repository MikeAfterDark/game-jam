...
function Physics:set_as_rectangle(w, h, body_type, tag)
	if not self.group then
		error("The GameObject must have a group defined for the Physics mixin to function")
	end
	self.tag = tag
...
function Physics:change_circle_radius(new_rs)
	...
	self.shape.rs = new_rs

	local shape = love.physics.newCircleShape(new_rs)
	self.fixture = love.physics.newFixture(self.body, shape)
	self.fixture:setUserData(self.id)
	self.fixture:setCategory(self.group.collision_tags[self.tag].category)
	self.fixture:setMask(unpack(self.group.collision_tags[self.tag].masks))

	if #self.group.trigger_tags[self.tag].triggers > 0 then
		self.sensor = love.physics.newFixture(self.body, shape)
		self.sensor:setUserData(self.id)
		self.sensor:setSensor(true)
	end
...
function Physics:draw_physics(color, line_width)
	if self.shape then
		self.shape:draw(color, line_width or 4)
	end
end
...
function Physics:get_objects_in_shape(shape, object_types)
	return table.select(self.group:get_objects_in_shape(shape, object_types), function(v)
		return v.id ~= self.id
	end)
end
...
function Physics:get_closest_object_in_shape(shape, object_types, exclude_list)
	...
	for i, object in ipairs(objects) do
		if not table.any(exclude_list, function(v)
...
function Physics:get_random_object_in_shape(shape, object_types, exclude_list)
	...
	if random_object then
		while table.any(exclude_list, function(v)
			return v.id == random_object.id
		end) and tries < 20 do
...
function Physics:move_towards_object(object, speed, max_time)
	if max_time then
		speed = self:distance_to_point(object.x, object.y) / max_time
	end
...
function Physics:move_towards_point(x, y, speed, max_time)
	if max_time then
		speed = self:distance_to_point(x, y) / max_time
	end
...
function Physics:move_towards_mouse(speed, max_time)
	if max_time then
		speed = self:distance_to_mouse() / max_time
	end
...
function Physics:separate(rs, class_avoid_list)
	...
	local objects = table.flatten(
		table.foreachn(class_avoid_list, function(v)
			return self.group:get_objects_by_class(v)
		end),
		true
	)
